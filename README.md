...
1. Запустить бота и показать главное меню.
2. Открыть пробный период и выбор тарифа.
3. Показать кнопки выбора платежного метода без проведения реальной оплаты.
4. Открыть профиль и экран инструкции подключения.
5. Открыть `/admin` и показать аналитику, управление тарифами, настройки сервиса и ручную выдачу подписки.
6. Коротко показать код: handlers, FSM-состояния, клавиатуры, сервисы и модели БД.

Такой формат демонстрирует работу логики без необходимости подключать реальные платежи и production VPN-сервер.

## Важные Примечания

- Реальные секреты должны храниться только в `.env`.
- Платежные интеграции вынесены в отдельные классы-провайдеры и могут быть включены при наличии credentials.
- Для production-запуска нужны PostgreSQL, Redis, настоящий токен Telegram-бота и настроенная 3X-UI панель.
